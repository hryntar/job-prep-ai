...
export function ResumePageClient({ jobInfoId }: { jobInfoId: string }) {
  ...
  function handleFileUpload(file: File | null) {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error(
        'Unsupported file format. Please upload a PDF, Word document, or text file.',
      );
      return;
    }
...
function FeedbackItem({
  ...
  if (message == null || name == null || type == null) return null;
  ...
  const getColors = () => {
    switch (type) {
      ...
      case 'minor-improvement':
        return 'bg-warning/10 border border-warning/50';
      default:
