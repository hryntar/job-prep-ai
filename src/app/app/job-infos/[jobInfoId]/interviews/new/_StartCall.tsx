...
export function StartCall({
  ...
  useEffect(() => {
    ...
    return router.push(`/app/job-infos/${jobInfo.id}/interviews/${interviewId}`);
...
function Controls() {
  const { mute, unmute, isMuted, micFft, disconnect, callDurationTimestamp } = useVoice();
  ...
  return (
    <div className="flex gap-5 rounded border px-5 py-2 w-fit sticky bottom-6 bg-background items-center">
      ...
      <div className="text-sm text-muted-foreground tabular-nums">{callDurationTimestamp}</div>
