    ...
    <div className="flex flex-col items-center gap-4 w-full max-w-[2000px] mx-auto grow h-screen-header">
      <div className="container flex gap-4 mt-4 items-center justify-between">
        <div className="grow basis-0">
          <BackLink href={`/app/job-infos/${jobInfo.id}`}>{jobInfo.name}</BackLink>
        </div>
        <Controls
          status={status}
      ...
      <QuestionContainer question={question} feedback={feedback} answer={answer} status={status} setAnswer={setAnswer} />
    </div>
  );
}
...
function QuestionContainer({
  ...
  return (
    <ResizablePanelGroup className="grow border-t">
    ...
    <div className="flex gap-2">
      {status === "awaiting-answer" ? (
        ...
        questionDifficulties.map((difficulty) => (
          <Button key={difficulty} size="sm" disabled={isLoading} onClick={() => generateQuestion(difficulty)}>
